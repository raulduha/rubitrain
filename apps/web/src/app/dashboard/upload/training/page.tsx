...
export default function UploadTrainingPage() {
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[#001e40]">Cargar Rutinas</h1>
        <p className="text-gray-500 text-sm mt-1">Importa sesiones y ejercicios desde un archivo Excel</p>
      </div>
        ...
        {[1, 2, 3].map(s => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
              step >= s ? 'bg-[#001e40] text-white' : 'bg-gray-100 text-gray-400'
            }`}>{s}</div>
      ...
      {step === 1 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-8">
          ...
          <p className="text-gray-500 text-sm mb-6">
            Usa el template oficial para asegurarte de que el formato sea correcto.
            Complétalo con tus datos y vuelve aquí para cargarlo.
          </p>
          <div className="bg-[#f8f9fa] rounded-xl p-4 mb-6 text-sm text-gray-600">
            <p className="font-medium text-[#001e40] mb-2">Columnas del template:</p>
            <p className="font-mono text-xs">Fecha · Titulo_Sesion · Tipo_Sesion · Grupo_Objetivo · Nombre_Ejercicio · Series · Reps · Distancia_m · Descanso_s · Intensidad · Notas</p>
          </div>
          <div className="flex gap-3">
            <a
              href="/api/templates/training"
              className="flex items-center gap-2 bg-[#001e40] text-white px-6 py-3 rounded-xl text-sm font-semibold hover:bg-[#003366] transition"
            >
              ⬇ Descargar Template Excel
            </a>
            <button
              onClick={() => setStep(2)}
              className="flex items-center gap-2 border border-gray-200 text-gray-600 px-6 py-3 rounded-xl text-sm font-medium hover:bg-gray-50 transition"
            >
              Ya tengo el archivo →
            </button>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-8">
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition mb-6 ${
              isDragActive ? 'border-[#0058bc] bg-blue-50' : 'border-gray-200 hover:border-[#0058bc] hover:bg-[#f8f9fa]'
            }`}
          >
            <input {...getInputProps()} />
              ...
              <div className="overflow-x-auto rounded-xl border border-gray-100">
                <table className="w-full text-xs">
                  <thead className="bg-[#f8f9fa]">
                    <tr>
                      {headers.map(h => (
                        <th key={h} className="text-left px-3 py-2 text-gray-500 font-semibold whitespace-nowrap">{h}</th>
                      ))}
                    </tr>
                  </thead>
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(1)} className="border border-gray-200 text-gray-600 px-5 py-2.5 rounded-xl text-sm hover:bg-gray-50">
              ← Atrás
            </button>
            <button
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-8">
          ...
          <div className="flex gap-4 mb-6">
            <div className="bg-[#f8f9fa] rounded-xl px-5 py-3 text-center">
              ...
              <p className="text-xs text-gray-400">sesiones creadas</p>
            </div>
            <div className="bg-[#f8f9fa] rounded-xl px-5 py-3 text-center">
              <p className="text-2xl font-bold text-[#001e40]">{result.exercises}</p>
              <p className="text-xs text-gray-400">ejercicios importados</p>
            </div>
            <div className="bg-[#f8f9fa] rounded-xl px-5 py-3 text-center">
          ...
          <button
            onClick={() => { setStep(1); setFile(null); setPreview([]); setResult(null); setTeamId('') }}
